...
export default function AdminDrivers() {
  ...
  const fetchAll = async () => {
    try {
      ...
      setDrivers(d.data ?? []);
      // only users who are not already drivers
                    ...
                    <td className="px-4 py-3 text-right">
                      {confirm === d.id ? (
                        ...
                        <span className="inline-flex gap-1 flex-wrap justify-end">
                          {d.status === 'PENDING' && (
                            <button onClick={() => action(d.id, 'approve', 'APPROVED')} className="rounded-lg border border-[#42d99a]/40 bg-[#42d99a]/10 px-2 py-1 text-[10px] text-[#42d99a] hover:bg-[#42d99a]/20">Approve</button>
                          )}
                          {d.status === 'PENDING' && (
                            <button onClick={() => action(d.id, 'reject', 'REJECTED')} className="rounded-lg border border-[#f87171]/40 bg-[#f87171]/10 px-2 py-1 text-[10px] text-[#f87171] hover:bg-[#f87171]/20">Reject</button>
                          )}
                          {d.status === 'APPROVED' && (
                            <button onClick={() => action(d.id, 'suspend', 'SUSPENDED')} className="rounded-lg border border-[#a78bfa]/40 bg-[#a78bfa]/10 px-2 py-1 text-[10px] text-[#a78bfa] hover:bg-[#a78bfa]/20">Suspend</button>
                          )}
                          {d.status === 'SUSPENDED' && (
                            <button onClick={() => action(d.id, 'approve', 'APPROVED')} className="rounded-lg border border-[#42d99a]/40 bg-[#42d99a]/10 px-2 py-1 text-[10px] text-[#42d99a] hover:bg-[#42d99a]/20">Restore</button>
                          )}
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setAddForm(f => ({ ...f, experienceYears: e.target.value }))}
                  placeholder="e.g. 5"
                  className="w-full rounded-xl border border-[#1e5670] bg-[#051a28] px-3 py-2.5 text-xs text-[#c8f2ff] placeholder-[#4a8fa8] outline-none focus:border-[#2ec5f5]"
                />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowAdd(false)} className="flex-1 rounded-xl border border-[#1e5670] py-2.5 text-xs font-bold uppercase tracking-wider text-[#7ea8bc] hover:text-[#c8f2ff] transition">
                  Cancel
                </button>
